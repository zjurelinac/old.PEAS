...
class Instruction( Group ):
    ...
    def encode( self, constants = None, line_number = None ):
        return None
...
class MemInstr( Instruction ):
    ...
    def encode( self, constants, line_number = None ):
        return [ self._opcodes[ self[ 0 ].contents ]
...
class MoveInstr( Instruction ):
    ...
    def encode( self, constants, line_number = None ):
        return [ '00000'
               ...
               + self[ 1 ].encode()
               + ( '' if isinstance( self[ 1 ], Integer ) else '0'*17 )
               ]
...
class JRInstr( Instruction ):
    ...
    def encode( self, constants, line_number ):
        has_condition = Constant in [ type( x ) for x in self.contents ]
        i = 3 if has_condition else 1
...
def parse_instruction( arguments ):
    ...
    if parsed[ 1 ]: raise SyntaxError( 'Extra tokens at the end of instruction' )
...
def get_int_from_tokens( tokens ):
    if isinstance( tokens[ 0 ], Integer ): value = int( tokens[ 0 ] )
    elif isinstance( tokens[ 1 ], Integer ): value = int( tokens[ 1 ] )
    else: raise TypeError( 'Cannot return an integer, wrong type of token' )
    return value

def get_data_size( instruction ):
    instr_name = instruction[ 0 ].contents

    if instr_name == 'DW': value = 4
    elif instr_name == 'DH': value = 2
    elif instr_name == 'DB': value = 1
    else: raise ValueError( 'Unknown data instruction' )
